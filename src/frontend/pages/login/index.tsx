...
const Login: NextPage = (): React.ReactElement<any, any> => {
    ...
    const [usernameInput, setUserInput]: [string, React.Dispatch<React.SetStateAction<string>>] = React.useState("");
    const [passwordInput, setPasswordInput]: [string, React.Dispatch<React.SetStateAction<string>>] = React.useState("");
    function loginSubmit(e: FormEvent<HTMLFormElement>): void {
        ...
        (async () => {
            ...
            try {
                ...
                alert("Login Server is currently offline.")
                console.error(err);
                return;
            }
                            ...
                            <Ext nav={true} layout={(): React.ReactElement<any, any> => 
                                <div className='login-page'>
                                    <form onSubmit={loginSubmit}>
                                        <input required type="text" placeholder="Email" value={usernameInput} onChange={(e: React.ChangeEvent<HTMLInputElement>):void => {setUserInput(e.target.value)}} />
